...
import { hasValidPreviousWorkoutTiming } from "@/lib/workout-session";
...
export default function ActiveActivityScreen({ session: initialSession, onChange, onFinish, onAbandon }: ActiveActivityScreenProps) {
  const [session, setSession] = useState(initialSession);
  const previous = session.entryMode === "previous";
  const timingValid = !previous || hasValidPreviousWorkoutTiming(session);
  async function change(next: WorkoutSession) { await onChange(next); setSession(next); }
  ...
  async function finishActivity() {
    if (!selectedIntensity || finishing || !timingValid) return;
    setFinishing(true);
    setError("");
    try {
      ...
      setError("Couldn’t save this activity. Your session is still here—please try again.");
    } finally {
      setFinishing(false);
    }
  }
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={previous ? "Log previous activity" : "Active activity"} className="active-activity-screen" style={{ position: "fixed", inset: 0, zIndex: 80, overflow: "hidden", background: "var(--background)" }}>
      <header className="active-activity-header">
        <p style={{ color: "#2e9a5b", fontSize: 11, fontWeight: 850, letterSpacing: "0.08em", textTransform: "uppercase", margin: "0 0 5px" }}>{previous ? "Log previous activity" : "Active activity"}</p>
        <div className="flex items-center justify-between" style={{ gap: 15 }}><h1 style={{ minWidth: 0, overflow: "hidden", color: "var(--primary)", fontSize: 23, lineHeight: 1.08, fontWeight: 850, letterSpacing: "-0.03em", textOverflow: "ellipsis", whiteSpace: "nowrap", margin: 0 }}>{session.workoutNameSnapshot}</h1>{!previous && <span className="active-activity-start-time"><MaterialIcon name="schedule" size={15} />Started {new Intl.DateTimeFormat(undefined, { hour: "numeric", minute: "2-digit" }).format(new Date(session.startedAt))}</span>}</div>
      </header>
