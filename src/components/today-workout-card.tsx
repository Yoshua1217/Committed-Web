...
import { ExerciseTargetPanel } from "@/components/exercise-coaching";
import { createWorkoutSession } from "@/lib/workout-session";
import { attachWorkoutCoaching, estimateWorkoutMinutes, exerciseTarget, formatTargetSet, workingSets } from "@/lib/workout-coaching";
...
export default function TodayWorkoutCard({ workout, sessions, completed, historyReady, enabled, active, onStart, onPreview, contextLabel }: {
  ...
  return <article className={styles.today}>
    <div className={styles.intro}><p className={`${styles.eyebrow} ${completed ? styles.positive : ""}`}><MaterialIcon name={completed ? "check_circle" : "fitness_center"} size={16} />{completed ? "Completed today" : contextLabel ?? "Today’s workout"}</p><h3>{workout.name}</h3><p className={styles.muted}>{workout.exercises.length} exercise{workout.exercises.length === 1 ? "" : "s"} · About {estimateWorkoutMinutes(workout, sessions)} min · Your saved routine</p>
      ...
      <div className={styles.actions}><button type="button" className={styles.primary} disabled={!enabled || busy} onClick={() => void start(workout)}>{busy ? "Starting…" : active ? "Resume" : completed ? "Start again" : "Start workout"}</button><button type="button" className={styles.secondary} onClick={onPreview}>Preview</button><button type="button" className={styles.quiet} disabled={!enabled || busy} onClick={() => setAdjusting("adjust")}>Adjust today</button><button type="button" className={styles.quiet} disabled={!enabled || busy} onClick={() => setAdjusting("shorten")}>Shorten session</button></div>
      {error && !adjusting && <p role="alert" className={styles.error}>{error}</p>}
    </div>
    <div className={styles.targets}><h4 className={styles.sectionTitle}>Your next targets</h4>{!historyReady ? <p className={styles.muted}>History is needed for progress-based guidance. You can still start and log normally.</p> : draft.exercises.length ? draft.exercises.map((exercise) => <div className={styles.exercise} key={exercise.exerciseId}><h5 className={styles.exerciseName}>{exercise.exerciseNameSnapshot}</h5>{exercise.coaching?.previous && <p className={styles.muted} style={{ marginBottom: 10 }}>Last time: {workingSets(exercise.coaching.previous).map((set) => formatTargetSet(set, exercise.loadType)).join(" · ") || "No completed working sets"}</p>}<ExerciseTargetPanel exercise={exercise} onIncrementChange={(value) => setIncrements((current) => ({ ...current, [exercise.exerciseId]: value }))} /></div>) : <p className={styles.muted}>Add exercises to this routine to see their targets.</p>}</div>
...
function AdjustTodayDialog({ workout, mode, busy, error, onClose, onStart }: { workout: WorkoutDefinition; mode: "shorten" | "adjust"; busy: boolean; error: string; onClose: () => void; onStart: (workout: WorkoutDefinition) => Promise<void> }) {
  ...
  return <WorkoutFlowDialog title={mode === "shorten" ? "A shorter session" : "Adjust today’s session"} description={mode === "shorten" ? "Choose the exercises and sets you have room for. Your saved routine stays the same." : "Adjust exercises, sets, or reps for this session only."} busy={busy} onClose={onClose}>
    <form className={styles.form} onSubmit={(event) => { event.preventDefault(); if (!plans.some((plan) => plan.included)) return; void onStart({ ...workout, exercises: plans.filter((plan) => plan.included).map(({ exerciseId, sortOrder, plannedSets, plannedReps }) => ({ exerciseId, sortOrder, plannedSets, plannedReps })) }); }}>
      {plans.map((plan, index) => <div key={plan.exerciseId} className={styles.adjustRow}><label><input type="checkbox" checked={plan.included} disabled={busy} onChange={(event) => setPlans((current) => current.map((item, i) => i === index ? { ...item, included: event.target.checked } : item))} />{names.find((item) => item.exerciseId === plan.exerciseId)?.exerciseNameSnapshot ?? plan.exerciseId}</label>{plan.included && <div className={styles.adjustFields}>{(["plannedSets", "plannedReps"] as const).map((field) => <label key={field}>{field === "plannedSets" ? "Sets" : "Reps"}<input required disabled={busy} type="number" min="1" max={field === "plannedSets" ? 20 : 100} step="1" value={plan[field] || ""} onChange={(event) => setPlans((current) => current.map((item, i) => i === index ? { ...item, [field]: Number(event.target.value) } : item))} /></label>)}</div>}</div>)}
