...
import { effortMinutes, formatEffort, taskBlock, taskGoalId } from "@/lib/task-planning";
...
export default function TaskDetailsModal({ isOpen, task, goals, buckets, onClose, onEdit }: TaskDetailsModalProps) {
  ...
  useEffect(() => {
    ...
    return subscribeToProjects(task.userId, setProjects);
  }, [isOpen, task?.userId]);
  useEffect(() => {
    if (!isOpen) return;
  ...
  const goal = goals.find((item) => item.id === taskGoalId(task, projects));
  ...
  return <div onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 12, background: "rgba(0, 0, 0, .6)", backdropFilter: "blur(8px)" }}>
    <section role="dialog" aria-modal="true" aria-label="Task details" style={{ width: "100%", maxWidth: 760, maxHeight: "calc(100dvh - 24px)", overflow: "hidden", border: "1px solid var(--border)", borderRadius: 22, background: "var(--surface)", boxShadow: "0 24px 80px rgba(0, 0, 0, .4)" }}>
      ...
      <div style={{ padding: "14px 22px", display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 8, maxHeight: "65dvh", overflowY: "auto" }}>
        {task.type === "task" && <>
          <Detail label="Project">{project ? <Link href="/dashboard/ideas">{project.name}</Link> : task.projectId ? "Linked project" : "No project"}</Detail>
          <Detail label="Estimated effort">{(effortMinutes(task) ? formatEffort(effortMinutes(task)) : "No estimate")}</Detail>
          <Detail label="Work block" wide>{block ? `${block.start.toLocaleString()} – ${block.end.toLocaleString()}` : effortMinutes(task) ? "Unscheduled · choose a time in Calendar" : "Add an effort estimate to plan a block"}</Detail>
        </>}
        ...
        <Detail label={task.projectId ? "Goal · inherited from project" : "Goal"}>{goal ? <span className="flex items-center" style={{ gap: 7 }}><MaterialIcon name={goal.iconName} size={16} color={bucket ? argbToHex(bucket.color) : "var(--secondary)"} />{goal.name}</span> : <span style={{ color: "var(--secondary)" }}>No goal selected</span>}</Detail>
