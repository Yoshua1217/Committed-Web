...
import CompletedSessionActions from "@/components/completed-session-actions";
import MaterialIcon from "@/components/material-icon";
import WorkoutPersonalRecordsDropdown from "@/components/workout-personal-records-dropdown";
import { WorkoutSession } from "@/lib/types";

interface CompletedWorkoutDetailModalProps { onRepeat?: (session: WorkoutSession) => Promise<void>; repeatLabel?: string; session: WorkoutSession; onClose: () => void; onDelete: (session: WorkoutSession) => Promise<void>; }
...
export default function CompletedWorkoutDetailModal({ session, onClose, onDelete, onRepeat, repeatLabel }: CompletedWorkoutDetailModalProps) {
  ...
  return <div role="dialog" aria-modal="true" aria-label={`${session.workoutNameSnapshot} history`} style={{ position: "fixed", inset: 0, zIndex: 71, overflow: "hidden", background: "var(--background)" }}>
    <div className="completed-workout-detail-sheet">
      ...
      <main className="completed-workout-detail-content">
        ...
        <div className="flex flex-col" style={{ gap: 22 }}>{session.exercises.slice().sort((a, b) => a.sortOrder - b.sortOrder).map((exercise, exerciseIndex) => <section key={exercise.exerciseId} style={{ padding: 16, border: "1px solid var(--border)", borderRadius: 17, background: "var(--surface)" }}><div className="flex items-center" style={{ gap: 10, marginBottom: 14 }}><span style={{ width: 27, height: 27, display: "grid", placeItems: "center", borderRadius: 9, background: "var(--surface-variant)", color: "var(--secondary)", fontSize: 12, fontWeight: 850 }}>{exerciseIndex + 1}</span><h3 style={{ color: "var(--primary)", fontSize: 16, fontWeight: 850, margin: 0 }}>{exercise.exerciseNameSnapshot}{exercise.addedDuringSession && <small style={{ display: "block", marginTop: 4, color: "#2e9a5b", fontSize: 11, fontWeight: 750 }}>Added during workout</small>}</h3></div><div className="completed-set-grid completed-set-heading"><span>Set</span><span>Weight</span><span>Reps</span><span>Status</span></div>{exercise.sets.map((set, setIndex) => { const record = session.personalRecords.find((item) => item.exerciseId === exercise.exerciseId && item.setIds[0] === set.id); return <div key={set.id} className="completed-set-grid completed-set-row" style={{ opacity: set.completed ? 1 : 0.6, border: record ? "1px solid #f5c84c40" : undefined, background: record ? "linear-gradient(#f5c84c08, #f5c84c08), var(--surface, #111)" : undefined }}><span>{setIndex + 1}</span><span>{exercise.loadType === "bodyweight" ? "BW" : set.weightLbs ?? "—"}</span><span>{set.reps ?? "—"}</span><span style={{ color: record ? "#d69e13" : set.completed ? "#2e9a5b" : "var(--secondary)", fontWeight: 850 }}>{record ? <span className="flex items-center justify-center" style={{ gap: 3 }}><MaterialIcon name="emoji_events" size={16} />PR</span> : set.completed ? "Done" : "Skipped"}</span></div>; })}</section>)}</div>
        <div aria-hidden="true" style={{ height: 30 }} />
      </main>
    </div>
    {optionsOpen && <CompletedSessionActions session={session} onClose={() => setOptionsOpen(false)} onRepeat={onRepeat} repeatLabel={repeatLabel} onDeleteRequest={() => { setOptionsOpen(false); setDeleteConfirmOpen(true); }} />}
