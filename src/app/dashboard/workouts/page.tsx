...
import { ActivityDefinition, ActivityIntensity, WorkoutDay, WorkoutDefinition, WorkoutSession } from "@/lib/types";
import { completeActivitySession, completeWorkoutSessionWithPersonalRecords, createActivitySession, createWorkoutSession, deleteCompletedSession, deleteWorkout, deleteWorkoutSession, saveWorkout, saveWorkoutSession, subscribeToActiveWorkoutSession, subscribeToCompletedWorkoutSessions, subscribeToWorkouts } from "@/lib/workouts-service";
...
function withHistoricalPersonalRecords(sessions: WorkoutSession[]): WorkoutSession[] {
  ...
  return sessions
    ...
    .map((session) => {
      if (session.sessionType === "activity") return { ...session, personalRecords: [] };
  ...
  return (
    <div className="workouts-page" style={{ padding: "32px 28px", maxWidth: 720, width: "100%" }}>
      <div className="mobile-page-header flex items-center justify-between" style={{ marginBottom: 24 }}>
        ...
        <button type="button" onClick={() => setCreateOpen(true)} style={{ background: "var(--primary)", color: "var(--background)", border: "none", borderRadius: 14, padding: "11px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>+ Create workout</button>
      </div>
          ...
          {historySessions.map((session) => {
            ...
            return <button key={session.id} type="button" onClick={() => setHistorySession(session)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "14px 14px 13px", border: "1px solid var(--border)", borderRadius: 14, background: "var(--background)", color: "var(--primary)", textAlign: "left", cursor: "pointer" }}>
              <span className="flex items-center" style={{ minWidth: 0, gap: 11 }}>
                {isActivity && <span style={{ width: 34, height: 34, display: "grid", placeItems: "center", flexShrink: 0, borderRadius: 11, background: "var(--surface-variant)", color: "#41e987" }}><MaterialIcon name={session.activityIconSnapshot ?? "directions_run"} size={19} /></span>}
                <span style={{ minWidth: 0 }}><strong className="flex items-center" style={{ gap: 6, fontSize: 15, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{!isActivity && session.personalRecords.length > 0 && <MaterialIcon name="emoji_events" size={18} color="#d69e13" />}{session.workoutNameSnapshot}</strong><small style={{ display: "block", marginTop: 4, color: "var(--secondary)", fontSize: 12, fontWeight: 600 }}>{isActivity ? `${session.activityCategorySnapshot ?? "Activity"}${intensity ? ` · ${intensity.label}` : ""} · ` : ""}{formatHistoryDate(session)} · {formatHistoryDuration(session)}</small></span>
              </span>
              <MaterialIcon name="chevron_right" size={21} color="var(--secondary)" />
            </button>;
          })}
      ...
      {createOpen && <CreateWorkoutModal isOpen userId={user?.uid ?? ""} nextSortOrder={workouts.length} onClose={() => setCreateOpen(false)} onCreate={saveWorkout} />}
      ...
      {activeSession?.sessionType === "activity" ? <ActiveActivityScreen session={activeSession} onFinish={finishActivity} onAbandon={abandonWorkout} /> : activeSession && <ActiveWorkoutScreen session={activeSession} onChange={saveWorkoutSession} onFinish={finishWorkout} onAbandon={abandonWorkout} />}
      ...
      {historySession?.sessionType === "activity" ? <CompletedActivityDetailModal session={historySession} onClose={() => setHistorySession(null)} onDelete={deleteCompletedSession} /> : historySession && <CompletedWorkoutDetailModal session={historySession} onClose={() => setHistorySession(null)} onDelete={deleteCompletedSession} />}
    </div>
  );
}
